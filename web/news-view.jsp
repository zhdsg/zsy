...
<div id="main" class="wrap">
		...
		<div class="news-list">
			...
			<ul>
				<C:forEach var = "news" items = "${newList}">
					<li><a href="/ncontent.servlet?id_news=${news.enId}" target="_blank">${news.enTitle}</a></li>
				</C:forEach>
			</ul>
		</div>
	</div>
	<div id="news" class="right-main">
		...
		<h1><li>   <c:if test="${id_ann!=0}">${annTitle}</c:if>	<c:if test="${id_news!=0}">${newTitle}</c:if> </li></h1>
		<div class="content">
<c:if test="${id_ann!=0}">	${annContent}</c:if> <c:if test="${id_news!=0}">${newContent}</c:if>
